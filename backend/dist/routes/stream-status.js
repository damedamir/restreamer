...
export const sendStreamStatusUpdate = (rtmpKey, isLive, viewers) => {
...
async function checkSRSStreamStatus(rtmpKey) {
    try {
        ...
        const stream = streams.find((s) => s.name === rtmpKey);
...
async function getSRSViewerCount(rtmpKey) {
    try {
        const response = await axios.get('http://srs:1985/api/v1/streams/');
        const streams = response.data.streams || [];
        const stream = streams.find((s) => s.name === rtmpKey);
